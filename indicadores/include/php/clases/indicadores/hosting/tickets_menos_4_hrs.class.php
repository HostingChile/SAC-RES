...
class tickets_menos_4_hrs extends Indicador {
	...
	private function calcularErrores($operador,$mes,$año){
		...
		$WHMCSManager = WHMCSManager::singleton();
		$query = "SELECT T.date as fecha_pregunta,IFNULL(R.date,NOW()) as fecha_respuesta, HOUR(IF(ISNULL(R.message),TIMEDIFF(NOW(),T.date),TIMEDIFF(R.date,T.date))) as hrs_respuesta
				 FROM tbltickets T 
					LEFT JOIN tblticketreplies R 
					ON R.id =                        
					   ( SELECT RR.id              
