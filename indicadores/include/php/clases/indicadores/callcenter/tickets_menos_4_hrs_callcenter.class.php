...
class tickets_menos_4_hrs_callcenter extends Indicador {
	...
	private function calcularAciertos($operador,$mes,$año){
		...
		foreach($result as $brand => $brand_results){
			foreach($brand_results as $row){
				...
				//si es que es un dia valido y estaba en turno, entonces as cierto
	...
	private function calcularErrores($operador,$mes,$año){
		...
		$WHMCSManager = WHMCSManager::singleton();
		$query_old = "SELECT T.tid,T.id,T.date as fecha_pregunta,IFNULL(R.date,NOW()) as fecha_respuesta, HOUR(IF(ISNULL(R.message),TIMEDIFF(NOW(),T.date),TIMEDIFF(R.date,T.date))) as hrs_respuesta
				 FROM tbltickets T 
					LEFT JOIN tblticketreplies R 
					ON R.id =                        
					   ( SELECT RR.id              
		...
		$query_new = "SELECT T.tid,T.id,T.date as fecha_pregunta,IFNULL(R.date,NOW()) as fecha_respuesta, HOUR(IF(ISNULL(R.message),TIMEDIFF(NOW(),T.date),TIMEDIFF(R.date,T.date))) as hrs_respuesta
				 FROM tbltickets T 
					LEFT JOIN tblticketreplies R 
					ON R.id =                        
					   ( SELECT RR.id              
		...
		foreach($result as $brand => $brand_results){
			foreach($brand_results as $row){
				...
				//si es que es un dia valido y estaba en turno, entonces es error
