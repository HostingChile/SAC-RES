    ...
    RGraph.Gauge = function (id, min, max, value)
    {
        ...
        */
        if (typeof(this.value) == 'object') {
            ...
            if (parseInt(this.value) > parseInt(this.max)) this.value = max;
            if (parseInt(this.value) < parseInt(this.min)) this.value = min;
        }
        ...
        */
        ...
        this.properties =
        {
            ...
            'chart.needle.colors':   ['#000000', 'red', 'green', 'yellow'],
        ...
        */
        ...
        this.DrawNeedle = function (value, color, index)
        {
			...
			//Comprobar si cae en un ticket grande y cambiar el valor
			if(value > this.min && prop['chart.scale.decimals'] == 0)
			{
				var numTicks = prop['chart.tickmarks.big'];
				var length = (this.max - this.min);
				var division = length / numTicks;

				for (var i=numTicks; i>=0; --i)
					if(value == (division * i).toFixed(prop['chart.scale.decimals']))
					{
						value = division * i;
						break;
					}
