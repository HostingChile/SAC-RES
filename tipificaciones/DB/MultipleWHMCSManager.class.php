...
class MultipleWHMCSManager
{
	...
	private function __construct()
	{
		...
		foreach ($conn_strings as $marca => $link) 
		{
			...
			{
			    debug("Falló la conexión con WHMCS de $marca ".$link[1]);
			    exit();
			}
		}
	}
	...
	public function execQuery($query, $brand)
	{
		if(isset($this->queries["$query-$brand"]))
			return $this->queries["$query-$brand"];
