        ...
        <div id="tree">
		    <ul>
		        <li>
		            ...
		            <ul>
		           	<?php
			        	foreach ($config_menu_options as $menu_option) {
		        			$checked_option = in_array("'{$menu_option['qname_id']}'", $tipos_llamadas_array) || $checked_all == "checked" ? "checked":"";
		        			echo "<li><input type='checkbox' value='{$menu_option['qname_id']}' class='checkbox_tipollamado' $checked_option />{$menu_option['queue']}</li>";
			        	}
			        ?>
		            </ul>
		        </li>
...
function createTable($listado_inicios,$total = true){
	...
	if($total)
		$table = "<table class=''><tr><th>Hora/Dia</th><th>Lunes</th><th>Martes</th><th>Miercoles</th><th>Jueves</th><th>Viernes</th><th>Sabado</th><th>Domingo</th><th>Total</th></tr>";
	else
		$table = "<table class=''><tr><th>Hora/Dia</th><th>Lunes</th><th>Martes</th><th>Miercoles</th><th>Jueves</th><th>Viernes</th><th>Sabado</th><th>Domingo</th></tr>";
	$sumadia = array();
	for($i = 0; $i < 24; $i++){
		$time = $i < 10 ? "0$i" : $i;
		for($j = 0;$j < 7;$j++){ 
		...
		$suma = $listado_inicios[$i][0] + $listado_inicios[$i][1] + $listado_inicios[$i][2] + $listado_inicios[$i][3] + $listado_inicios[$i][4] + $listado_inicios[$i][5] + $listado_inicios[$i][6] ;
		if($total)
			$table .= "<tr><td>$time:00:00 - $time:59:59</td><td style='background-color:$color0'>{$listado_inicios[$i][0]}</td>
															 ...
															 <td style='background-color:$color3'>{$listado_inicios[$i][3]}</td>
															 <td style='background-color:$color4'>{$listado_inicios[$i][4]}</td>
															 <td style='background-color:$color5'>{$listado_inicios[$i][5]}</td>
															 <td style='background-color:$color6'>{$listado_inicios[$i][6]}</td>
															 <td>$suma</td></tr>";
		else
			$table .= "<tr><td>$time:00:00 - $time:59:59</td><td style='background-color:$color0'>{$listado_inicios[$i][0]}</td>
															 <td style='background-color:$color1'>{$listado_inicios[$i][1]}</td>
															 ...
															 <td style='background-color:$color4'>{$listado_inicios[$i][4]}</td>
															 <td style='background-color:$color5'>{$listado_inicios[$i][5]}</td>
															 <td style='background-color:$color6'>{$listado_inicios[$i][6]}</td></tr>";
	}
	if($total){
		$sumatotal = $sumadia[0]+$sumadia[1]+$sumadia[2]+$sumadia[3]+$sumadia[4]+$sumadia[5]+$sumadia[6];
		$table.="<tr><th>Total</th><th>{$sumadia[0]}</th><th>{$sumadia[1]}</th><th>{$sumadia[2]}</th><th>{$sumadia[3]}</th><th>{$sumadia[4]}</th><th>{$sumadia[5]}</th><th>{$sumadia[6]}</th><th>$sumatotal</th></tr>";
	}
	else
		$table.="<tr><th></th><th>{$sumadia[0]}</th><th>{$sumadia[1]}</th><th>{$sumadia[2]}</th><th>{$sumadia[3]}</th><th>{$sumadia[4]}</th><th>{$sumadia[5]}</th><th>{$sumadia[6]}</th></tr>";
	$table .= "</table>";
...
function obtenerLlamadasPerdidas($mes,$año,$tipos_llamadas){
	...
	$link = mysqli_connect("190.153.249.226","root","hosting.,12","qstats") or die("Error de conexión con la Base de Datos: " . mysqli_error($link));
	
	$where = $tipos_llamadas != "TODAS" ? " AND qname IN ($tipos_llamadas) " : "";

	$query = "SELECT WEEKDAY(datetime) as dia, HOUR(datetime) AS hora, SUM(perdida) AS perdidas, SUM(recibida) as recibidas, 
				ROUND(SUM(perdida) * 100 / (SUM(perdida) + SUM(recibida)),2) AS porcentaje 
			FROM
			(
				SELECT *, IF(qevent = 1 OR qevent = 13, 1, 0) AS perdida, IF(qevent = 10, 1, 0) AS recibida FROM queue_stats 
				WHERE 
				MONTH(datetime) = $mes AND 
				YEAR(datetime) = $año  AND
				(qevent = 1 OR qevent = 10 OR qevent = 13)
				$where
			) AS tmp
			GROUP BY WEEKDAY(datetime), HOUR(datetime)";
